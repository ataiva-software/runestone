...
const configReferenceTemplate = `# Configuration Reference
...
func (g *Generator) generateConfigurationReference() error {
	data := struct {
		GeneratedAt string
	}{
	...
	content, err := g.executeTemplate(configReferenceTemplate, data)
	if err != nil {
		return err
	}

	return g.writeFile("configuration-reference.md", content)
}
